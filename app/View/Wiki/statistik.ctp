...
if(!empty($wiki_pages)){
    ...
    foreach($wiki_pages as $wikiPage){
        echo '<li><a href="getpagehtml/'.$wikiPage['WikiPage']['title'].'">'.$wikiPage['WikiPage']['title']."</a>";
        if(!empty($wikiPage['WikiElement'])){
            echo "<ul>";
            foreach($wikiPage['WikiElement'] as $wikiElement){
                echo '<li><a href="gethtml/'.$wikiPage['WikiPage']['title'].'/'.$wikiElement['element_id'].'">'.$wikiElement['element_id'].'</a></li>';
            }
            echo "</ul>";
        }
        echo '</dt>';
